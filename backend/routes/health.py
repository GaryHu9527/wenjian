...
def health():
    ...
    return jsonify(success=True, data={"backend": "ok", "ai_available": bool(config.ai_api_key and config.ai_base_url and config.ai_model) and not config.use_mock_ai, "zhihu_available": bool(config.zhihu_api_key) and not config.use_mock_zhihu, "ai_mode": "mock" if config.use_mock_ai else "remote", "classical_nlp_provider": nlp["provider"], "version": "1.2.0"})
