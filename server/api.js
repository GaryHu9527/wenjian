...
import { searchCorpus } from '../src/services/corpus.js'
import { buildZhihuQuery, discussionCategory, demoDiscussions } from '../src/services/discussion.js'
...
async function zhihu(env,payload) {
 if(payload.engine==='mock') return demoDiscussions(payload.text,payload.article)
 const query=buildZhihuQuery(payload.text,payload.article), key=`zhihu:${query}`, now=Date.now(), stored=cache.get(key)
 ...
 const items=raw.slice(0,5).map((i,n)=>({id:i.ContentID||n,title:i.Title||'未命名内容',excerpt:i.ContentText||'',url:i.Url,author:i.AuthorName||'知乎用户',author_url:i.AuthorUrl||i.AuthorURL,category:discussionCategory(i),stats:{vote_up_count:i.VoteUpCount||0,comment_count:i.CommentCount||0}}))
